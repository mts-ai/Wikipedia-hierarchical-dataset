...
Each file will contain several documents in the format:
    ...
    <doc id="" revid="" url="" title="" category="">
        ...
        </doc>
...
class Extractor(object):
    ...
    def write_output(self, out, text):
        ...
        if options.write_json:
            ...
            if options.print_revision:
                header = '<doc id="%s" revid="%s" url="%s" title="%s" category="%s">\n' % (self.id, self.revid, url, self.title, self.category)
            else:
                header = '<doc id="%s" url="%s" title="%s" category="%s">\n' % (self.id, url, self.title, self.category)
            footer = "\n</doc>\n"
            if out == sys.stdout:   # option -a or -o -
...
def process_dump(input_file, template_file, out_file, file_size, file_compress,
    ...
    for page_data in pages_from(input):
        ...
        if keepPage(ns, catSet, page):
            ...
            #print("CatSet: ",catSet)
            #print("CatSet type: ", type(catSet))
            #print("title", title)
            catSet = "|".join(list(catSet))
            #print("catSet", catSet)
...
def extract_process(opts, i, jobs_queue, output_queue):
    ...
    while True:
        ...
        if job:
            ...
            try:
                #id, revid, title, lines, catSet
                #print("CAT:", job[5])
                e = Extractor(id=id, revid=revid, title=title, lines=page, catSet=catSet) # (id, revid, title, page)
                page = None              # free memory
                e.extract(out)
                text = out.getvalue()
            except:
